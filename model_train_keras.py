...
def createBidirectionalLSTM(segment_time_size,
                            ...
                            batch_size,
                            X_train, y_train,
                            X_val, y_val, feature_amount, N_CLASSES,
                            visualize=False):
    """
    ...
    model.add(Bidirectional(LSTM(n_hidden_neurons,
                            ...
                            activation="tanh"), input_shape=(segment_time_size, feature_amount),
                            merge_mode='sum'))
    model.add(Bidirectional(LSTM(n_hidden_neurons)))
    model.add(Dropout(droput_rate))
    model.add(Dense(N_CLASSES, activation='sigmoid'))
    adam_optimizer = Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999, decay=0.0, amsgrad=False)
...
if __name__ == '__main__':
    ...
    data_left = data_left.drop(['glove'], axis=1)
    data_convoluted, labels = get_convoluted_data(data_left)
    ...
    data_right = data_right.drop(['glove'], axis=1)
    data_convoluted, labels = get_convoluted_data(data_right)
    X_train, X_val, y_train, y_val = train_test_split(data_convoluted,
                                                        labels, test_size=TEST_SIZE,
                                                        random_state=RANDOM_SEED,
                                                        shuffle=True)
